...
class GliderSimulator():
    ...
    def simulate_without_steering(self, estimate_next_position, params):
        """Run simulation only to locate glider.

        Args:
            estimate_next_position(func): Student submission function to estimate next glider position.
        ...
        try:
            while steps < params['max_steps']:
                ...
                result = estimate_next_position(target_height, target_meas, studentMapFunc, other_info)
                ...
                if len(result) == 3:
                    ...
                    print("estimate_next_position did not return correct number of return values!")
    ...
    def simulate_with_steering(self, next_turn_angle, params):
        """Run simulation to allow glider to be steered.

        Args:
            next_turn_angle(func): Student submission function for gliders next turn angle.
            params(dict): Test parameters.
        ...
        try:
            while steps < params['max_steps']:
                target_meas = target.sense()
                target_height = target.get_height()
                result = next_turn_angle(target_height, target_meas, studentMapFunc, other_info)
                if len(result) == 3:
                    ...
                    print("next_turn_angle did not return 2 or 3 return values!")
...
class PartATestCase(CaseRunner):
    ...
    def setUp(self):
        ...
        self.student_method = marsglider.estimate_next_position
...
class PartBTestCase(CaseRunner):
    ...
    def setUp(self):
        ...
        self.student_method = marsglider.next_turn_angle
